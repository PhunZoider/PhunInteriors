...
-- ---------------------------------------------------------------------------
-- A filterable, sortable definition list, VENDORED from PhunMart2
-- (client/PhunMart_Client/ui/base/list_panel.lua).
--
-- What changed: the namespace, the derive name, the recompile event became
-- Core.events.OnRegistryChanged, and the cross-reference require became
-- ui/state.lua -- which supplies Core.isShippedKey, Core.isOverriddenKey and a
-- Core.references stub whose canBeReferenced answers false, switching the
-- "used by" feature off. See state.lua for why.
--
-- It brings the filter box, the sortable columns and the stripe down the left
-- of a row that says whether a definition is stock, modified or custom. Those
-- last two are exactly the states the override layer produces, which is most
-- of the reason this file came across rather than being rewritten.
--
-- Vendored rather than depended on, with the same trade as form_panel beside
-- it: no hard dependency, at the cost of a fork that has to be maintained by
-- hand. Read it as somebody else's code.
-- ---------------------------------------------------------------------------
...
local ListPanel = ISPanel:derive("PhunInteriorsListPanel")
...
Events[Core.events.OnRegistryChanged].Add(function()
...
function ListPanel:createChildren()
    ...
    if not self._noFilter then
        local filterLblW = getTextManager():MeasureStringX(UIFont.Small, getText("IGUI_PhunInteriors_Lbl_Filter")) + 8
        local filterLbl = ISLabel:new(PAD, PAD, BUTTON_HGT, getText("IGUI_PhunInteriors_Lbl_Filter"), 0.8, 0.8, 0.8, 1, UIFont.Small, true)
    ...
    if self._defKind then
        ...
        self._onlyChangedW = getTextManager():MeasureStringX(UIFont.Small, getText("IGUI_PhunInteriors_Lbl_OnlyChanged")) +
                                 BUTTON_HGT + PAD
    end
    ...
    if self._hasTemplates then
        ...
        ttick:initialise()
        ttick:instantiate()
        ttick:addOption(getText("IGUI_PhunInteriors_Lbl_HideTemplates"), nil)
        ttick:setSelected(1, true)
        ttick.tooltip = getText("IGUI_PhunInteriors_Tip_HideTemplates")
        ...
        self._hideTemplatesW = getTextManager():MeasureStringX(UIFont.Small,
            getText("IGUI_PhunInteriors_Lbl_HideTemplates")) + BUTTON_HGT + PAD
    end
    ...
    if self:showsReferences() then
        ...
        refsBtn._tip = getText("IGUI_PhunInteriors_Tip_UsedBy")
    ...
    local closeBtnW = math.max(math.floor(70 * FONT_SCALE),
        getTextManager():MeasureStringX(UIFont.Small, getText("IGUI_PhunInteriors_Btn_Close")) + PAD * 2)
    local closeBtn = ISButton:new(0, PAD, closeBtnW, BUTTON_HGT, getText("IGUI_PhunInteriors_Btn_Close"), self,
        self.requestClose)
    closeBtn:initialise()
...
function ListPanel:addNameColumn(color)
    self:addListColumn(getText("IGUI_PhunInteriors_Col_Name"), 0, {
        text = function(d)
...
function ListPanel:updateReferenceLine()
    ...
    if self._refsOrphan then
        ...
        self._refsText = getText("IGUI_PhunInteriors_Lbl_UsedByNothing")
    else
        self._refsText = getText("IGUI_PhunInteriors_Lbl_UsedBy", Core.references.summarise(found))
    end
end
...
function ListPanel:prerender()
    ...
    local filterLblW = hasFilter and
                           (getTextManager():MeasureStringX(UIFont.Small, getText("IGUI_PhunInteriors_Lbl_Filter")) + 8) or 0
    ...
    -- A PhunInteriors addition to the vendored file, kept to four lines so the
    -- rest stays a near-verbatim copy. A subclass wanting controls of its own
    -- between the description and the list defines layoutHeaderRow(y, width),
    -- positions its widgets and returns how tall it made itself. The slots tab
    -- uses it for the room and vehicle pickers; filter TABS could not, being a
    -- row of toggles over a fixed handful of values rather than a dropdown
    -- over eighty rooms.
    local headerH = 0
    if self.layoutHeaderRow then
        headerH = self:layoutHeaderRow(PAD + descH + tabsH, w) or 0
    end

    local listY = PAD + descH + tabsH + headerH + HEADER_HGT
