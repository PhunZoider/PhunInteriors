...
-- ---------------------------------------------------------------------------
-- A form, VENDORED from PhunMart2
-- (client/PhunMart_Client/ui/base/form_panel.lua), near verbatim.
--
-- What changed: the namespace, the derive name, and PhunMart.utils.deepCopy /
-- deepEquals became Core.tools.deepCopy / deepEquals. Nothing else -- the file
-- reaches for four font constants and nothing of PhunMart's own vocabulary,
-- which is why vendoring it turned out to cost far less than it looked.
--
-- Vendored rather than depended on: this mod has no hard dependencies and is
-- not growing one for a UI framework. The duplication is deliberate, and the
-- cost is real -- a fix to PhunMart's copy has to be carried over by hand, and
-- the two will drift. It was taken because the alternative was reinventing
-- combo, tick, range, picker and list fields plus validation, and because an
-- admin running several Phun mods should not have to learn a second kind of
-- form to do the same job.
--
-- Read it as somebody else's code. If something here needs changing, consider
-- whether it should change in PhunMart2 first.
-- ---------------------------------------------------------------------------
...
local FormPanel = ISCollapsableWindowJoypad:derive("PhunInteriorsFormPanel")
...
function FormPanel:addPickerField(key, label, opts)
    ...
    table.insert(self._fields, {
        ...
        label = label,
        _value = opts.value,
        _display = opts.display or getText("IGUI_PhunInteriors_Lbl_None"),
        onPick = opts.onPick,
        hint = opts.hint,
        group = opts.group,
        visible = true
...
function FormPanel:_applyStep()
    ...
    if self._applyBtn then
        self._applyBtn:setTitle(last and getText("IGUI_PhunInteriors_Btn_Create") or getText("IGUI_PhunInteriors_Btn_Next"))
    end
...
function FormPanel:setPickerValue(key, value, displayText)
    ...
    if f and f.type == "picker" then
        f._value = value
        f._display = displayText or getText("IGUI_PhunInteriors_Lbl_None")
...
function FormPanel:setFieldInherited(key, sourceLabel)
    ...
    f._inheritNote = getText("IGUI_PhunInteriors_Hint_Inherited", tostring(sourceLabel))
...
function FormPanel:_isShowingInherited(f)
    ...
    if not f._inheritCaptured then
        ...
        if f.type == "list" then
            loaded = PhunInteriors.tools.deepCopy(loaded)
        end
    ...
    if type(current) == "table" or type(f.inheritedValue) == "table" then
        return PhunInteriors.tools.deepEquals(current, f.inheritedValue)
    end
    ...
    for _, f in ipairs(self._fields) do
        ...
        if f.inheritedFrom then
            consider(((f.hint or "") ~= "" and (f.hint .. "  ") or "") ..
                         getText("IGUI_PhunInteriors_Hint_Inherited", tostring(f.inheritedFrom)))
        else
            consider(f.hint)
        end
        if f.type == "separator" then
...
function FormPanel:createChildren()
    ...
    self._applyBtn = ISButton:new(btnX, 0, btnW, ROW_H, getText("IGUI_PhunInteriors_Btn_Apply"), self,
    ...
    self._cancelBtn = ISButton:new(btnX + btnW + btnGap, 0, btnW, ROW_H, getText("IGUI_PhunInteriors_Btn_Cancel"), self,
    ...
    if self._onDelete then
        ...
        local label = self._deleteLabel or getText("IGUI_PhunInteriors_Btn_Delete")
    ...
    if self._steps then
        self._backBtn = ISButton:new(PAD, 0, btnW, ROW_H, getText("IGUI_PhunInteriors_Btn_Back"), self,
...
function FormPanel:_createField(f)
    ...
    elseif f.type == "picker" then
        ...
        local pickBtnW = math.max(math.floor(60 * FONT_SCALE), getTextManager():MeasureStringX(UIFont.Small, getText(
            "IGUI_PhunInteriors_Btn_Pick")) + PAD * 2)
        f._pickBtn = ISButton:new(0, 0, pickBtnW, ROW_H, getText("IGUI_PhunInteriors_Btn_Pick"), self, function(btn)
            if f.onPick then
                f.onPick(self, f)
            end
        end)
    ...
    elseif f.type == "list" then
        ...
        if f._editable then
            f._addBtn = addRowButton(getText("IGUI_PhunInteriors_Btn_Add"), false, function()
                if f.onAdd then
                    f.onAdd(self, f)
                end
            end)

            f._editBtn = addRowButton(getText("IGUI_PhunInteriors_Btn_Edit"), true, function()
                local sel = f._list.selected
            ...
            f._removeBtn = addRowButton(getText("IGUI_PhunInteriors_Btn_Delete"), true, function()
                local sel = f._list.selected
                if sel and sel > 0 and f._items[sel] then
                    if f.onRemove then
...
local function validateField(form, f)
    ...
    if f.required and blank then
        return getText("IGUI_PhunInteriors_Err_Required")
    end
...
function FormPanel._builtinChecks(form, f, value)
    if f.type == "range" then
        ...
        if (value.min ~= "" and not mn) or (value.max ~= "" and not mx) then
            return getText("IGUI_PhunInteriors_Err_Numeric")
        end
        if f.requireBoth and (not mn or not mx) then
            return getText("IGUI_PhunInteriors_Err_RangeBoth")
        end
        if mn and mx and mx < mn then
            return getText("IGUI_PhunInteriors_Err_RangeOrder")
        end
        if f.min and ((mn and mn < f.min) or (mx and mx < f.min)) then
            return getText("IGUI_PhunInteriors_Err_Min", tostring(f.min))
        end
        ...
        if f.max and ((mn and mn > f.max) or (mx and mx > f.max)) then
            return getText("IGUI_PhunInteriors_Err_Max", tostring(f.max))
        end
        if f.integer and ((mn and mn % 1 ~= 0) or (mx and mx % 1 ~= 0)) then
            return getText("IGUI_PhunInteriors_Err_Integer")
        end
